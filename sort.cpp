...
int main(int argc, char** argv)
{
   ...
   /* create a back up array to store the original data before sorting */
   int* B=new int [size];             
   for (int i = 0; i < size; i++)
   		B[i]=A[i];
   ...
   for (int i=0;i<1;i++)             // number of iterations
   {
		s->resetNumCmps();    

   		/* reset data by overwriting it with back up array */  
   		for (int i = 0; i < size; i++)
   		A[i]=B[i];     
		...
		clock_t start = clock();                // start clock
   		...
   		accmulator += finish-start;
   	    //accmulator += difference of start and end
   }	
   ...
   if (op.showTime())
      cout << "Running time: "
           << (double)(accmulator)*1000/CLOCKS_PER_SEC
           << " ms" << endl;
